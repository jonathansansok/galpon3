//frontend\src\components\ui\secuestroElementos\ComponentesModal.tsx
import { FC, useState, useEffect } from "react";
import { phoneBrands } from "./MarcasCel";
interface ComponentesModalProps {
  initialComponentes: string;
  onSave: (componentes: string) => void;
}
...
const ComponentesModal: FC<ComponentesModalProps> = ({
  initialComponentes,
  onSave,
}) => {
  const [componentes, setComponentes] = useState<Componente[]>([]);
  const [tipoComponente, setTipoComponente] = useState<string>("");
  const [mostrarParteEspecifica, setMostrarParteEspecifica] =
  ...
  useEffect(() => {
    if (initialComponentes) {
      const data = JSON.parse(initialComponentes);
      setComponentes(data);
      if (data.length > 0) {
        const lastComponente = data[data.length - 1];
        setTipoComponente(lastComponente.tipoComponentes);
        setTipoEspecifico(lastComponente.tipoEspecifico);
        setMarca(lastComponente.marca);
        setModelo(lastComponente.modelo);
        setMedida(lastComponente.medida);
        setDetalles(lastComponente.detalles);
        setDentroCelda(lastComponente.dentroCelda);
        setSectorHabitacional(lastComponente.sectorHabitacional);
        setDonde(lastComponente.donde);
        setPabellon(lastComponente.pabellon);
        setOtroSector(lastComponente.otroSector);
        setOtroDetalle(lastComponente.otroDetalle);
      }
    }
  }, [initialComponentes]);

  const openModal = () => {
    const modal = document.getElementById("componentes-modal");
    if (modal) {
  ...
  const handleTipoComponenteChange = (
    e: React.ChangeEvent<HTMLSelectElement>
  ) => {
    const value = e.target.value;
    setTipoComponente(value);
    setMostrarParteEspecifica(value.startsWith("Parte de"));
    setMostrarSelectMarca(value === "Parte de Movil celular");
  };
  ...
  const handleSaveChanges = () => {
    onSave(JSON.stringify(componentes));
    closeModal();
  };

  return (
    <div id="componentes-group">
      <button
        data-modal-target="componentes-modal"
        data-modal-toggle="componentes-modal"
          ...
          <div className="relative bg-white rounded-lg shadow-2xl dark:bg-gray-800">
            <div className="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-700">
              <h3 className="text-xl font-semibold text-gray-900 dark:text-white">
                Registro de Secuestro de Componentes
              </h3>
              <button
                type="button"
                className="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white"
                data-modal-hide="componentes-modal"
                onClick={closeModal}
              >
                <svg
                  className="w-3 h-3"
            ...
            <div className="p-4 md:p-5 space-y-4">
              <h4 className="text-lg font-semibold text-gray-900 dark:text-white">
                Componentes Secuestrados:
                <span id="contadorComponentes" className="ml-2">
                  {componentes.length}
                </span>
              </h4>
              <ul id="listaComponentes" className="list-group space-y-2">
                {componentes.map((componente, index) => (
                  <li
                    key={index}
                    className="list-group-item p-2 bg-green-100 rounded-md dark:bg-gray-700"
                  >
                    <strong>Tipo:</strong> {componente.tipoComponente} -{" "}
                    {componente.tipoEspecifico} <br />
                    <strong>Marca:</strong> {componente.marca} <br />
                    <strong>Modelo:</strong> {componente.modelo} <br />
                    <strong>Medida:</strong> {componente.medida} cm <br />
                    <strong>Detalles:</strong>{" "}
                    {componente.detalles || "Ninguno"} <br />
                    <strong>Dentro de Celda:</strong> {componente.dentroCelda}{" "}
                    <br />
                    {componente.dentroCelda === "Sí" && (
                      <p>
                        <strong>Celda:</strong> {componente.pabellon}
                      </p>
                    )}
                    <strong>Sector Habitacional:</strong>{" "}
                    {componente.sectorHabitacional} <br />
                    {componente.sectorHabitacional === "Otro" && (
                      <p>
                        <strong>Otro Sector:</strong> {componente.otroSector}
                      </p>
                    )}
                    <strong>Donde (específico):</strong> {componente.donde}{" "}
                    <br />
                    {componente.donde === "Otro" && (
                      <p>
                        <strong>Detalle:</strong> {componente.otroDetalle}
                      </p>
                    )}
                    <button
              ...
              <div className="form-group">
                ...
                <select
                  ...
                  onChange={handleTipoComponenteChange}
                  className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md dark:bg-gray-700 dark:border-gray-600 dark:text-gray-300"
                >
                  <option value="">Seleccione</option>
                  <option value="Parte de Movil celular">
                    Parte de Movil celular
                  </option>
                  <option value="Parte de TV">Parte de TV</option>
                  <option value="Parte de Radio">Parte de Radio</option>
                  <option value="Parte de Consola de videojuego">
                    Parte de Consola de videojuego
                  </option>
                  <option value="Parte de Heladera">Parte de Heladera</option>
                  <option value="Parte de Microondas">
                    Parte de Microondas
                  </option>
                  <option value="Parte de Freezer">Parte de Freezer</option>
                </select>
                {mostrarParteEspecifica && (
                  <div className="form-group">
                    <label
                      htmlFor="parteEspecifica"
                      className="block text-sm font-medium text-gray-700 dark:text-gray-300"
                    >
                      Especifique componente:
                    </label>
                    <input
                      type="text"
                      id="parteEspecifica"
              ...
              <div className="form-group">
                ...
                <textarea
                  id="detalles"
                  placeholder="Detalles adicionales sobre el componente"
                  rows={3}
                  value={detalles}
