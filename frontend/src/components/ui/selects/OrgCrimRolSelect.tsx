// frontend/src/components/ui/OrgCrimRolSelect.tsx
...
const opcionesOrgCrim: Option[] = [
  ...
  { value: "No consta", label: "No consta" },
  { value: "NO pertenece a una organización criminal", label: "NO pertenece a una organización criminal" },
];

interface OrgCrimRolSelectProps {
  orgCrim: string;
  cualorg: string;
  rol: string;
  onChange: (orgCrim: string, cualorg: string, rol: string) => void;
}

const OrgCrimRolSelect: React.FC<OrgCrimRolSelectProps> = ({ orgCrim, cualorg, rol, onChange }) => {
  ...
  const [rolValue, setRolValue] = useState<string>(rol);
  ...
  const handleOrgCrimChange = (value: string) => {
    ...
    if (value === "NO pertenece a una organización criminal") {
      ...
      onChange(value, cualOrgValue, rolValue);
    }
  };

  const handleCualOrgChange = (value: string) => {
    setCualOrgValue(value);
    onChange(selectedOrgCrim, value, rolValue);
  };

  const handleRolChange = (value: string) => {
    setRolValue(value);
    onChange(selectedOrgCrim, cualOrgValue, value);
  };
    ...
    <div>
      ...
      {(selectedOrgCrim === "Pertenece a una organización criminal" || selectedOrgCrim === "En investigación") && (
        <>
          ...
          <div className="form-group" id="rolInput">
            <input
...
export default OrgCrimRolSelect;