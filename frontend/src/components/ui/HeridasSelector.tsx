...
interface HeridasSelectorProps {
  onSelect: (selectedHeridas: Herida[]) => void;
  defaultHeridas?: Herida[];
}

export const HeridasSelector = ({ onSelect, defaultHeridas = [] }: HeridasSelectorProps) => {
  const [selectedHeridas, setSelectedHeridas] = useState<Herida[]>(defaultHeridas);
  const [zona, setZona] = useState("");
  const [tipoHerida, setTipoHerida] = useState("");
  ...
  const tipoHeridaOptions = ["Autoagresiva", "Arma blanca", "Arma de fuego"];

  const handleAddHerida = () => {
    if (zona && tipoHerida && detalle) {
      const newHerida = { zona, type: tipoHerida, detail: detalle };
      setSelectedHeridas((prevSelected) => [...prevSelected, newHerida]);
      setZona("");
      setTipoHerida("");
      setDetalle("");
    }
  ...
  const handleRemoveHerida = (index: number) => {
    setSelectedHeridas((prevSelected) =>
      prevSelected.filter((_, i) => i !== index)
    );
  };

  const closeModal = () => {
    const modal = document.getElementById("heridas-modal");
    if (modal) {
  ...
  const handleSaveChanges = () => {
    onSelect(selectedHeridas);
    closeModal();
  };

  return (
    <div id="heridas-group">
      <button
        data-modal-target="heridas-modal"
        data-modal-toggle="heridas-modal"
                ...
                <div className="w-3/5 flex-grow">
                  <div className="flex flex-col space-y-2">
                    ...
                    <select
                      ...
                      className="w-full p-1 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500"
                    >
                      <option value="">Tipo de herida</option>
                      {tipoHeridaOptions.map((option) => (
                        <option key={option} value={option}>
                          {option}
                        </option>
                      ))}
                    ...
                    <input
                      ...
                      className="w-full p-1 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500"
                      placeholder="Detalles de la herida"
                    />
                    <button
                      type="button"
                      onClick={handleAddHerida}
                  ...
                  <ul className="list-disc list-inside mt-4">
                    {selectedHeridas.map((herida, index) => (
                      <li key={index} className="flex items-center">
                        <button
                          type="button"
                          onClick={() => handleRemoveHerida(index)}
