...
export function SearchBarPreingresos({ data, onSearchResults }: SearchBarProps) {
  ...
  const normalizedData = Array.isArray(data)
    ? data.map((item) => {
        ...
        if (item.fechaHoraIng) {
          normalizedItem.fechaHoraFormatted = formatDate(new Date(item.fechaHoraIng));
        }
  ...
  const handleSearchClick = async () => {
    ...
    results = results.filter((result) => {
      const itemDate = result.item.fechaHoraIng ? new Date(result.item.fechaHoraIng) : null;
