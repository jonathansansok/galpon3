//frontend\src\app\portal\eventos\riesgos\Riesgos.api.ts
...
export async function getRiesgos() {
  try {
    const res = await fetch(`${BACKEND_URL}/api/riesgos`, {
      cache: "no-store",
    });
...
export async function getRiesgo(id: string) {
  try {
    const res = await fetch(`${BACKEND_URL}/api/riesgos/${parseInt(id, 10)}`, {
      cache: "no-store",
    });

    if (!res.ok) {
      throw new Error(`Error al obtener riesgo con id ${id}`);
    }

    return await res.json();
  } catch (error) {
    console.error(`Error al obtener riesgo con id ${id}:`, error);
    throw error;
  }
}

export async function createRiesgo(riesgo: any) {
  try {
    console.log("[CSRF] Verificando cookies disponibles en el navegador:", document.cookie);
...
export async function deleteRiesgo(id: string) {
  try {
    ...
    console.error(`Error al eliminar riesgo con id ${id}:`, error);
    return {
      success: false,
      data: null,
      error: (error as Error)?.message || 'Error desconocido',
...
export async function updateRiesgo(id: string, newRiesgo: any) {
  try {
    console.log("[CSRF] Verificando cookies disponibles en el navegador:", document.cookie);
    ...
    console.error(`Error al actualizar riesgo con id ${id}:`, error);
    return {
      success: false,
      data: null,
      error: (error as Error)?.message || 'Error desconocido',
