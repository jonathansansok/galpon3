...
export async function getTraslados() {
  try {
    const res = await fetch(`${BACKEND_URL}/api/traslados`, {
      cache: "no-store",
    });
...
export async function getTraslado(id: string) {
  try {
    const res = await fetch(`${BACKEND_URL}/api/traslados/${parseInt(id, 10)}`, {
      cache: "no-store",
    });

    if (!res.ok) {
      throw new Error(`Error al obtener traslado con id ${id}`);
    }

    return await res.json();
  } catch (error) {
    console.error(`Error al obtener traslado con id ${id}:`, error);
    throw error;
  }
}
...
export async function deleteTraslado(id: string) {
  try {
    ...
    console.error(`Error al eliminar traslado con id ${id}:`, error);
    return {
      success: false,
      data: null,
      error: (error as Error)?.message || 'Error desconocido',
...
export async function updateTraslado(id: string, formData: FormData) {
  try {
    console.log("[CSRF] Verificando cookies disponibles en el navegador:", document.cookie);
    ...
    console.error(`Error al actualizar traslado con id ${id}:`, error);
    return {
      success: false,
      data: null,
      error: (error as Error)?.message || 'Error desconocido',
