...
*/
-- DropForeignKey
ALTER TABLE `presupuestos` DROP FOREIGN KEY `Presupuestos_movilId_fkey`;

-- DropIndex
DROP INDEX `Presupuestos_movilId_fkey` ON `presupuestos`;
...
-- AlterTable
ALTER TABLE `presupuestos` MODIFY `movilId` INTEGER NULL;
...
-- AddForeignKey
ALTER TABLE `Presupuestos` ADD CONSTRAINT `Presupuestos_movilId_fkey` FOREIGN KEY (`movilId`) REFERENCES `Temas`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;
