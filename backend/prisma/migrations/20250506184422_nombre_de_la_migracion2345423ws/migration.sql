/*
  ...
  - You are about to drop the column `ingresoId` on the `moviles` table. All the data in the column will be lost.
  ...
  - You are about to alter the column `fechaHora` on the `temas` table. The data in that column could be lost. The data in that column will be cast from `DateTime(0)` to `DateTime`.
...
*/
-- DropForeignKey
ALTER TABLE `moviles` DROP FOREIGN KEY `Moviles_ingresoId_fkey`;

-- DropIndex
DROP INDEX `Moviles_ingresoId_fkey` ON `moviles`;
...
-- AlterTable
ALTER TABLE `moviles` DROP COLUMN `ingresoId`;
...
-- AlterTable
ALTER TABLE `temas` MODIFY `fechaHora` DATETIME NULL;
