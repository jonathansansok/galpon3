...
export class PresupuestosService {
  ...
  async create(createPresupuestoDto: CreatePresupuestoDto) {
    try {
      ...
      const result = await this.prismaService.presupuestos.create({
        data: {
          monto: createPresupuestoDto.monto,
          estado: createPresupuestoDto.estado || 'Pendiente',
          observaciones: createPresupuestoDto.observaciones,
        },
      });
  ...
  async findAll() {
    try {
      return await this.prismaService.presupuestos.findMany();
    } catch (error) {
    ...
    try {
      const result = await this.prismaService.presupuestos.update({
        ...
        data: {
          ...
          monto: updatePresupuestoDto.monto, // Aseguramos que `monto` sea un número
        },
      });
